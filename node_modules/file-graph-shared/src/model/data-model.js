...
class DataModel extends ListenableObject {
    ...
    addNode(node) {
        ...
        node.tags.forEach(tagName => {
            ...
            tag.nodes.forEach(rightFile => {
                const edgeId = Edge.generateId(node, rightFile);
                if (this._edges[edgeId] == null) {
                    const edge = new Edge(node, rightFile, {
                        tags: [tag]
                    });
    ...
    updateFile(sendFile) {
        const file = this._nodes[sendFile.getId()];
    ...
    static loadFromJSON({ tags, nodes }) {
        ...
        nodes.forEach(n => dataModel.addNode(Node.fromJSON(n, {
            getTagForName: (name) => dataModel._tags[name]
        })));
